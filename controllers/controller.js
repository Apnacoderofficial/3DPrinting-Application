...
exports.adminlist = async (req, res) => {
  try {
    checkAuth(req, res, async () => {
     ...
     res.render('Admin-List', { users, roles });
    });
  } catch (error) {
...
exports.getAddAdminForm = async (req, res) => {
  checkAuth(req, res, async () => {
    try {
      const roles = await Role.find();
      res.render('Embed-Admin', { roles,type:'add' });
    } catch (error) {
...
exports.addAdmin = async (req, res) => {
  checkAuth(req, res, async () => {
    try {
      ...
      mailer(req.body.email, '', '', `Welcome ${req.body.name} to Our 3D Printing Service! 😊 `, `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <p>Hi ${name},</p>
      ...
      <p>To get started, please visit our website and log in using the provided credentials:</p>
      <p><a href="${baseUrl}/login">Log in to Your Account</a></p>
      ...
      <p>We look forward to serving you and seeing the amazing projects you create!</p>
...
exports.getEditAdminForm = async (req, res) => {
  checkAuth(req, res, async () => {
    try {
      ...
      res.render('Embed-Admin', { user, roles ,type:'edit'});
    } catch (error) {
...
exports.addStlFile = async (req, res) => {
  checkAuth(req, res, async () => {
    try {
      res.render('addStlFile', { existingFileData : undefined ,type:'Add' });
    } catch (error) {
      console.error('Error fetching roles:', error);
      res.status(500).send('Internal Server Error');
    }
...
exports.editStlFile = async (req, res) => {
  checkAuth(req, res, async () => {
    try {
      ...
      res.render('addStlFile', { existingFileData, type:'Edit' });
    } catch (error) {
      console.error('Error fetching roles:', error);
      res.status(500).send('Internal Server Error');
    }
...
exports.postFileAuth = async (req, res) => {
  try {
    ...
    const baseUrl = req.protocol + '://' + req.get('host');
    openSTLFileIn3DSprint(`${baseUrl}/uploads/${existingFileData.stlFile}`);
...
async function launch3DSprint(localFilePath) {
  const executablePath = 'C:\\Program Files\\3D Systems\\3D Sprint 5.2.0.1034\\3DSprint.exe';
